...
if __name__ == '__main__':
    ...
    RKNN_PATH="/rknn_rgbt/model/rgbt_ca_rtdetrv2_ours_add_original_op19_three_outputs_conv_int8.rknn"
    ...
    Acc_eval=False
    SNAPSHOT_INT8_DIR="./SNAPSHOT_INT8"
    DATASET='/rknn_rgbt/convert/datasets_128.txt'
    ...
    cv2.imwrite("intput_letterbox_img_rgb.jpg",img_rgb)
    # 查看图像的基本信息
    print("Image shape:", img_rgb.shape)  # 输出图像的尺寸 (height, width, channels)
    print("Image data type:", img_rgb.dtype)  # 输出图像的数据类型 (通常为 uint8)
    pixel_value = img_rgb[256, 320]  # 注意：OpenCV中索引是 (y, x)
    print("Pixel value at ({}, {}): {}".format(320, 256, pixel_value))
    ...
    cv2.imwrite("intput_letterbox_img_t.jpg",img_t)
    ...
    if Acc_analysis:
        ...
        Ret = rknn.accuracy_analysis(inputs=[img_rgb,img_t],output_dir=SNAPSHOT_INT8_DIR)
        if ret != 0:
    ...
    if not Acc_analysis and not Acc_eval:
        ...
        # outputs = rknn.inference(inputs=[img_rgb,img_t],data_format=["nchw","nchw"])
        ...
        if Easy_postprocess:
            ...
            cv2.imwrite("/rknn_rgbt/convert/out_rgb_int8_rgbt.jpg", img_rgb_plot)
            cv2.imwrite("/rknn_rgbt/convert/out_t_int8_rgbt.jpg", img_t_plot)
        else:
